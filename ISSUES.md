...
> Current version: **v0.15.0** (server — plugin unchanged since v0.14.0, no `.rbxm`
> rebuild needed). After upgrading, restart the MCP client (npx pulls latest).

## ✅ What landed in v0.15.0
- **MCP SDK 1.30.0 + spec-2026-07-28 audit** — `@modelcontextprotocol/sdk` ^1.29.0 → ^1.30.0. The 1.x monolith tops out at 1.30.0 and still speaks the `initialize`-handshake era (protocol 2025-11-25); the 2026-07-28 stateless spec lives in the NEW split v2 packages (`@modelcontextprotocol/server`/`/client`/`/core` 2.0.0) — **migration deferred** until the v2 line bakes. 1.30.0 itself is maintenance-only for us (zod error formatting, stdio receive-buffer cap — our tool outputs are already capped, Streamable-HTTP fixes we don't use; the Express bridge is the plugin long-poll channel, NOT an MCP transport, so untouched). Zero code changes forced. Full writeup: `docs/MCP-SPEC-2026-07-28.md`.
- **Production server observability (BETA endpoints)** — 2 new tools (83 → 85). `get_server_logs` (per-server errors/warnings + stack traces from the LIVE game; without `serverId` it lists active servers to pick from) and `get_server_history` (rolling 30-day terminated/crashed/OOM server list with status/uptime/terminationTime/last-known memory+fps). Both hit the Open Cloud **Server Management** API: `GET /server-management/v1/universes/{u}/places/{p}/versions/{v}/game-servers[/{jobId}/logs]` (scope `universe:read`, 100 req/min/key), confirmed against the official openapi.json in Roblox/creator-docs; the `{versionNumber}` path segment auto-resolves to the latest published version via the experimental place-version-history API (`universe.place:read`) or an explicit `version` arg. Read tools — visible in `TUFAN_READONLY`, registered in the core tier. **Verified keyless** (schema + missing-key error path); **not yet exercised against the live API**.
- **`datastore` quota awareness** — since 2026-07-29 Open Cloud DataStore calls share the SAME unified per-experience request budget as in-game DataStore calls (base 300 + CCU × multiplier/request-type/minute). The tool description now warns about it, successful responses append a `quota:` line whenever the API sends `x-ratelimit-*` headers (the spec documents none — surfaced only when actually present), and the stale "~300 req/min/universe" 429 text was corrected. No throttling added — awareness only.
- **Per-project AI context** — new `project_context` tool (85 → 86, core tier): a persistent per-place markdown blob at `~/.tufan-blox-bridge/context/<placeId>.md` the agent reads at session start BEFORE exploring (fewer tokens, fewer false bug reports). `get` works in read-only mode; `set`/`append` (dated entries) write-gated via `requireWritable()` like `script_source`. 64 KB cap. An explicit numeric PlaceId works with no Studio connected. Verified: 12/12 keyless round-trip incl. read-only gating.
- **Distribution** — new CLI command `tufan-blox-bridge install-plugin` (argv intercepted before the MCP server starts; no-args stdio behavior byte-identical): downloads `TufanBloxBridge.rbxm` from the LATEST GitHub release into the Studio Plugins folder (`%LOCALAPPDATA%\Roblox\Plugins` on Windows, `~/Documents/Roblox/Plugins` on macOS, clear manual instructions elsewhere). The root one-liners (`install.ps1` / `install.sh`) rewritten: Node ≥18 check → `npm i -g tufan-blox-bridge` → `install-plugin` → auto `claude mcp add` when the CLI is present + paste-ready config JSON for other clients. PS5.1-safe (pure ASCII, NativeCommandError-proof). **Verified live on Windows**: v0.14.0 plugin installed, byte-identical to the repo build.

### Known uncertainties (v0.15.0)
- **CEL filter field name** — `get_server_history`/`get_server_logs` filter with `server_status` (per the devforum announcement's wording); the openapi spec only says "CEL filtering is supported on all fields". If Roblox actually expects `status`, the raw `filter` arg is the escape hatch. One live call with a real key settles it.
- **No live Open Cloud calls made** for the two server-management tools (no key/published universe in the dev environment) — paths/params/schemas are pinned from the official spec, but a keyed smoke against a real universe is still owed.
...
- **AI 3D-asset pipeline (v0.11.0)** — 5 new tools (76 → 81), server-only. `generate_asset`: text prompt (or image) → Meshy AI textured mesh → headless-Blender Roblox-readiness lint + auto-fix (decimate to ≤20k tris — Roblox silently mangles above; downscale textures to ≤1024; warn on multi-material) → Open Cloud upload (reuses the `import_file` core, now extracted to `src/openCloud.ts`) → inserted as a Model. Raw stages exposed too: `meshy_generate`/`meshy_task` (preview→refine auto-chain, remesh, taskId resume tokens, fresh signed-URL re-GET before every download, credit-confirm gate ~30cr/$0.60 with `TUFAN_MESHY_AUTOCONFIRM=1` opt-out) and `blender_run`/`blender_process` (execFile + `@@TUFAN_RESULT@@` sentinel protocol; 8 canned ops incl. recursive `split_chunks` and extension-gated Cell-Fracture `fracture`). Env: `TUFAN_MESHY_KEY`, `TUFAN_BLENDER_PATH` (auto-detect), `TUFAN_ASSET_DIR`. **Verified:** Blender chain 11/11 live on Blender 5.1 headless + 16/16 keyless validation. **Pending:** Meshy/full-pipeline E2E awaits a Meshy Pro API key (`meshy-e2e.mjs` / `generate-e2e.mjs` are authored and key-gated); Cell Fracture extension not yet installed (clean install-instructions error verified).
