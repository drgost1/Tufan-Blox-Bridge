...
Connect Claude Code / Cursor and get 86 tools to drive Studio: read/write scripts, create & edit instances, get/set properties (full round-trip incl. particle curves and gradients), inspect the tree, run Luau, batch ops, subtree snapshots, perf/responsive/security audits, read logs, insert assets — plus two-way file sync (files↔Studio for code) and per-place git history (commit/log/diff/restore/branch).
...
- `TUFAN_TOOLSET=core` — expose only the 27 everyday core tools (default `full` = all 86).
